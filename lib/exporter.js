...
class PgExporter {
  ...
  init(variables, totalRows) {
    if (this.initialized) {
      throw new Error('Export was already initialized');
    }
    ...
    if (this.useGeom && (!this.longitudeVarIndex || !this.latitudeVarIndex)) {
      throw new Error('Configured variables to be used as a geometry but variables were not given');
    }

    if (!variables.TIME && !this.useGeom) {
